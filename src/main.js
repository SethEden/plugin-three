...
// Then let the plugin do final organization of that data and again pass it back to Haystacks so that the ap usng Haystacks can make calls to the plugin functions and all its data.
...
// The plugin Haystacks instance would again have to lod all of the Haystacks configuration data, and meta-data and be completely bootstrapped yet again.
...
// #1: Add a data parameter to the function below: initializePlugin that allows for the application instance of Haystacks, when it calls tis function to load this plugin,
...
// #2: In the host application, immediately after creating the instance of Haystacks, the instance object could be stored in the appConfig object that is then passed along to the Haystacks bootStrapper.
// Then the instance object of Haystacks could e stored n the Haystacks "D" as a context object.
...
// and they should behave exactly the same way, and be capable fo doing exactly the same things.
 ...
 * Including beng able to make calls back to Haystacks, for the purpose of loading and parsing files, or any number of other operations that need to be done.
